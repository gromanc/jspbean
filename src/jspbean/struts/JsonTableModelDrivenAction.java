...
@Action(value = "jsonTableModelDriven", results = {
  ...
  interceptorRefs = {
    @InterceptorRef("modelDriven"),@InterceptorRef("params"), @InterceptorRef("annotationWorkflow")
//    @InterceptorRef(value="defaultStack", params={
//        "params.acceptParamNames", "(\\[\\d+\\]\\.)*\\w+((\\.\\w+)|(\\[\\d+\\])|(\\(\\d+\\))|(\\['\\w+'\\])|(\\('\\w+'\\)))*"
//      })
  })
public class JsonTableModelDrivenAction extends ActionSupport implements ModelDriven {

  private static final long serialVersionUID = 5078264277068533593L;
  private static final Log log              = LogFactory.getLog(JsonTableModelDrivenAction.class);
...
//    total = (int) Math.ceil((double) records / (double) rows);
  ...
  @Action(value = "editTableModelDriven", results = {
    @Result(type = "json", params = {"includeProperties", "gridModel\\[\\d+\\]\\.id, gridModel\\[\\d+\\]\\.name, total, records, rows, page, sidx, searchField, searchString",  "excludeNullProperties", "true"})},
    interceptorRefs = {
      @InterceptorRef("modelDriven"),@InterceptorRef("params")
    })
